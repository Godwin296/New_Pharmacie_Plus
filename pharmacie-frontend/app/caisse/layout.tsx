          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed top-0 right-0 h-full w-80 bg-white dark:bg-slate-900 z-70 shadow-2xl flex flex-col p-8 border-l border-emerald-500/20">
                ...
                <div className="flex justify-between items-center mb-10 text-emerald-700">
                    ...
                    <button aria-label="Fermer le menu" onClick={() => setIsMenuOpen(false)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-xl border-none cursor-pointer"><X size={20}/></button>
                </div>
                ...
                <button aria-label="Déconnexion" onClick={() => setShowLogoutConfirm(true)} className="w-full mt-auto flex items-center justify-center gap-3 text-red-500 font-black p-5 rounded-2xl border-none bg-red-50 dark:bg-red-500/10 cursor-pointer uppercase text-[10px] tracking-widest">
